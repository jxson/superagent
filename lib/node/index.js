...
Request.prototype.query = function(obj){
  ...
  // HACK: When using the nock or replay modules requests get overriden
  // in a way that will not have the `req.path` attribute set. Since the
  // `path` attribute is not advertised on the http core module
  // (http://nodejs.org/api/http.html) and the query params can get ignored
  // when matching the fake requests this seemed like the best place to put
  // this check. As opposed to dropping it in nock.
  if (req.path) req.path += (~req.path.indexOf('?') ? '&' : '?') + query;
  return this;
};
